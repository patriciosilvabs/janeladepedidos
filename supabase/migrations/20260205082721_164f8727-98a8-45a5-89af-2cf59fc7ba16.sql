-- Adicionar colunas para palavras-chave configuráveis
...
-- Atualizar função para usar keywords dinâmicas
      ...
      LOOP
        ...
        IF v_option_name != '' THEN
          ...
          IF v_edge_arr IS NOT NULL THEN
            ...
            LOOP
              -- Se keyword começa com #, verificar se option_name começa com #
              IF v_keyword = '#' THEN
                IF v_option_name LIKE '#%' THEN
                  v_is_edge := true;
                  EXIT;
                END IF;
              -- Senão, verificar se contém a keyword (case insensitive)
          ...
          IF NOT v_is_edge AND v_flavor_arr IS NOT NULL THEN
            ...
            -- Verificar também no grupo (para keywords como "Sabor")
