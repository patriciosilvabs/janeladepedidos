...
DECLARE
  ...
  v_item_qty integer;
  v_qty_idx integer;
  -- New variables for flavor splitting
  v_flavor_array text[];
  v_edge_array text[];
...
BEGIN
  ...
  LOOP
    ...
    -- ============================================================
    -- FLAVOR SPLITTING LOGIC: detect multiple flavors and split
    -- ============================================================
    v_flavor_array := string_to_array(v_flavors, E'\n');
    v_flavor_count := COALESCE(array_length(v_flavor_array, 1), 0);
    
    -- Only split if there are multiple flavors (combo scenario)
    IF v_flavor_count > 1 THEN
      -- Split edges into array too
      ...
      LOOP
        -- For each flavor, create a separate order_item
        ...
        LOOP
          ...
          -- Pair edge with flavor (1:1 mapping by index)
          ...
          -- Determine if this individual item has an edge
          -- (for sector routing purposes)
          v_has_edge := (v_single_edge IS NOT NULL AND v_single_edge != '');
          
          -- Sector assignment (same logic as before, per individual item)
          ...
          -- Route through edge sector if this individual pizza has an edge
    ...
    ELSE
      -- ============================================================
      -- ORIGINAL LOGIC: single flavor or no flavors - keep as-is
      -- ============================================================
      FOR v_qty_idx IN 1..v_item_qty
      LOOP
        IF p_default_sector_id IS NULL THEN
