...
-- Update create_order_items_from_json to support hybrid classification via _type field
-- Options tagged with _type by edge functions take priority over keyword matching
...
BEGIN
  ...
  LOOP
    ...
    IF v_item->'options' IS NOT NULL AND jsonb_array_length(v_item->'options') > 0 THEN
      ...
      LOOP
        ...
        IF v_option_name != '' THEN
          ...
          -- HYBRID: Check pre-classified _type from edge function first
          v_pre_type := v_option->>'_type';
          
          IF v_pre_type IS NOT NULL THEN
            -- Use pre-classified type (from option_group_id mapping)
            IF v_pre_type = 'edge' THEN
              v_is_edge := true;
            ELSIF v_pre_type = 'flavor' THEN
              v_is_flavor := true;
            END IF;
            -- _type = 'complement' falls through (not edge, not flavor = complement)
          ELSE
    ...
    -- Check product name for edge keywords too
    IF NOT v_has_edge AND v_edge_arr IS NOT NULL THEN
      FOREACH v_keyword IN ARRAY v_edge_arr
      LOOP
        IF v_keyword != '#' AND COALESCE(v_item->>'name', '') ILIKE '%' || v_keyword || '%' THEN
          v_has_edge := true;
          EXIT;
        END IF;
      END LOOP;
    ...
    -- Calculate production sector (load balancing)
    ...
    -- EDGE ROUTING: item com borda COMEÇA em BORDAS, depois vai pra PRODUÇÃO
